...
class Grid_Data(Base):
    ...
    uid = Column(BIGINT, primary_key=True)
    meta_id = Column(BIGINT, ForeignKey('grid_meta.uid'))
    var_id = Column(BIGINT, ForeignKey('grid_vars.uid'))
    date = Column(BIGINT, ForeignKey('grid_dates.uid'))
    rast = Column(Raster)
...
class Regions(Base):
    ...
    uid = Column(BIGINT, primary_key=True)
    meta_id = Column(BIGINT, ForeignKey('regions_meta.uid'))
